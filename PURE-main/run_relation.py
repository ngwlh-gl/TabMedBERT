...
from relation.models import BertForRelation, AlbertForRelation,RobertaForRelation
...
def convert_examples_to_features(examples, label2id, max_seq_length, tokenizer, special_tokens, unused_tokens=True,model='bert',padding_idx=0):
    """
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        tokens = [CLS] if ('roberta' not in model and 'infoxlm' not in model) else ['<s>']
        ...
        tokens.append(SEP) if ('roberta' not in model and 'infoxlm' not in model) else tokens.append('</s>')
        ...
        padding = [padding_idx] * (max_seq_length - len(input_ids))
        segment_padding=[0] * (max_seq_length - len(input_ids))
        input_ids += padding
        input_mask += padding
...
def main(args):
    ...
    elif 'roberta' in args.model or 'infoxlm' in args.model:
        RelationModel = RobertaForRelation
    else:
    ...
    # train set
    if args.do_train:
        train_dataset, train_examples, train_nrel = generate_relation_data(args.train_file, args.task,use_gold=True, context_window=args.context_window)
    # dev set
    if (args.do_eval and args.do_train) or (args.do_eval and not(args.eval_test)):
        eval_dataset, eval_examples, eval_nrel = generate_relation_data(os.path.join(args.entity_output_dir, args.entity_predictions_dev), args.task,use_gold=args.eval_with_gold, context_window=args.context_window)
    # test set
    if args.eval_test:
        test_dataset, test_examples, test_nrel = generate_relation_data(os.path.join(args.entity_output_dir, args.entity_predictions_test), args.task,use_gold=args.eval_with_gold, context_window=args.context_window)
    ...
    if args.do_eval and (args.do_train or not(args.eval_test)):
        eval_features = convert_examples_to_features(
            eval_examples, label2id, args.max_seq_length, tokenizer, special_tokens, unused_tokens=not(args.add_new_tokens),model=args.model,padding_idx=tokenizer.pad_token_id)
        logger.info("***** Dev *****")
    ...
    if args.do_train:
        train_features = convert_examples_to_features(
            train_examples, label2id, args.max_seq_length, tokenizer, special_tokens, unused_tokens=not(args.add_new_tokens),model=args.model,padding_idx=tokenizer.pad_token_id)
        ...
        model = RelationModel.from_pretrained(
            args.model, num_rel_labels=num_labels)
        
        if args.use_our_pretrain:
            if 'roberta' not in args.model and 'infoxlm' not in args.model:
                special_tokens_dict = {'additional_special_tokens': ['[sos]']}
                num_added_toks = tokenizer.add_special_tokens(special_tokens_dict)
                logger.info(f'Adding {num_added_toks} new tokens {"[SOS]"}!')
                print(f'Adding {num_added_toks} new tokens {"[SOS]"}!')
                if args.pubmedbert_large:
                    pass
                else:
                    model.bert.resize_token_embeddings(len(tokenizer))
                checkpoint = torch.load(args.ours_pretrain_ckpt, map_location='cpu')
                ...
                for k,v in checkpoint.items():
                    if k.startswith('model.roberta.'):
                        length=len('model.roberta.')
                        state_dict[k[length:]]=v
                msg = model.roberta.load_state_dict(state_dict, strict=False)
                logger.info(msg)
                print(msg)
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_batches):
                ...
                # input_positions=torch.arange(input_ids.shape[1]).unsqueeze(0).to(input_ids.device)
    ...
    if args.do_eval:
        ...
        if args.eval_test:
            ...
            eval_features = convert_examples_to_features(
                test_examples, label2id, args.max_seq_length, tokenizer, special_tokens, unused_tokens=not(args.add_new_tokens),model=args.model,padding_idx=tokenizer.pad_token_id)
            eval_nrel = test_nrel
            logger.info(special_tokens)
            logger.info("***** Test *****")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ours_pretrain_ckpt", default=None, type=str,
                        help="Path to our pre-trained model")
    ...
    parser.add_argument("--pubmedbert_large", action='store_true', help='whether load pretrained model parameters.')
    ...
    parser.add_argument('--task', type=str, default=None, required=True, choices=['ace04', 'ace05', 'scierc','cdr','ade','chr','biored','ddi','gad','chemprot','ppi','drugprot'])
