...
SRC_TGT_PAIRS={
    ...
    'cdr':set([('Chemical','Disease')]),
    'ade':set([('Adverse-Effect','Drug')]),
    'ddi':set([('DRUG','DRUG')]),
    'gad':set([('GENE','DISEASE')]),
    'chemprot':set([('GENE','CHEMICAL')]),
    'ppi':set([('PROTEIN','PROTEIN')]),
    'drugprot':set([('CHEMICAL','GENE')]),
}

def generate_relation_data(entity_data, task,use_gold=False, context_window=0):
    """
    ...
    src_tgt_pairs = SRC_TGT_PAIRS[task]
    nner, nrel = 0, 0
    max_sentsample = 0
    samples = []
    count_wrong=0
        ...
        for i, sent in enumerate(doc):
            ...
            for x in range(len(sent_ner)):
                for y in range(len(sent_ner)):
                    ...
                    # if task=='cdr':
                    #     if not (sub.label=='Chemical' and obj.label=='Disease'):
                    #         continue
                    # if task=='ade':
                    #     if not (sub.label=='Adverse-Effect' and obj.label=='Drug'):
                    #         continue
                    if task=='biored':
                        if sub.label=='SequenceVariant':
                            sub.label='GeneOrGeneProduct'
                        if obj.label=='SequenceVariant':
                            obj.label='GeneOrGeneProduct'
                        if not ((sub.label,obj.label) in src_tgt_pairs):
