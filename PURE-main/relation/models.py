...
# from modeling_kebio import KebioModel
...
class BertForRelation(BertPreTrainedModel):
    def __init__(self, config, num_rel_labels):
        ...
        if config.architectures and 'Kebio' in config.architectures[0]:
            from modeling_kebio import KebioModel
            # config.max_position_embeddings=args.max_position_embeddings
            self.bert = KebioModel(config)
        else:
            # if config.vocab_size==30522 and pubmedbert_large:
            #     config.vocab_size=28896
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, sub_idx=None, obj_idx=None, input_position=None):
        ...
        if self.config.architectures and 'Kebio' in self.config.architectures[0]:
...
class RobertaForRelation(RobertaPreTrainedModel):
    def __init__(self, config, num_rel_labels):
        super(RobertaForRelation, self).__init__(config)
        self.num_labels = num_rel_labels
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, sub_idx=None, obj_idx=None, input_position=None):
        outputs = self.roberta(input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask, output_hidden_states=False, output_attentions=False, position_ids=input_position)
        sequence_output = outputs[0]
        sub_output = torch.cat([a[i].unsqueeze(0) for a, i in zip(sequence_output, sub_idx)])
        obj_output = torch.cat([a[i].unsqueeze(0) for a, i in zip(sequence_output, obj_idx)])
        rep = torch.cat((sub_output, obj_output), dim=1)
