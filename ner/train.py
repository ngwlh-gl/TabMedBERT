...
# from process_pubmed.after_process import inter
...
def evaluate(encoder, mlp, crf, data_loader, output_filepath=None, verbose=False):
    with torch.no_grad():
        ...
        for sentences in data_loader:
            sentence_outputs,masks=encoder.forward(sentences)
            preds = crf.viterbi_tags(mlp(sentence_outputs), sentences)
            for s, pred in zip(sentences, preds):
                for t, p in zip(s, pred):
                    t.set_label("pred", p)
                s.clear_embeddings()
        logger.info(f"Finish evaluation: {time.time() - start_time} s")
        print(f"Finish evaluation: {time.time() - start_time} s")
    ...
    print(f"micro-avg: acc {metric.micro_avg_accuracy()} - micro-avg-f1-score {metric.micro_avg_f_score()}")
   

    if verbose:
        logger.info("\t".join(["Class", "TP", "FP", "FN", "TN", "Precision", "Recall", "F1"]))
        print("\t".join(["Class", "TP", "FP", "FN", "TN", "Precision", "Recall", "F1"]))
        for n in metric.get_classes():
            ...
            print("%s\t%d\t%d\t%d\t%d\t%.4f\t%.4f\t%.4f" % (n, tp, fp, fn, tn, p, r, f))
...
def final_test(args, encoder, mlp, crf, test_data, name,mode):
    logger.info("-" * 100)
...
def train_epoch(args, encoder, mlp, crf, optimizer, train_data, epoch, category2mentions, label2tokens,src_mlp=None,src_crf=None,src_train=None,mt_model=None):
    logger.info("-" * 100)
    print("-" * 100)
    logger.info(f"# sentences and augmented sentences: {len(train_data)}")
    print(f"# sentences and augmented sentences: {len(train_data)}")
    ...
    for i in range(num_batches):
        ...
        sequence_output,masks=encoder.forward(sentences)
        features = mlp(sequence_output)
        ...
        if i % logging_intervals == 0:
            ...
            print(f"epoch {epoch}/{args.max_epochs} - batch {i + 1}/{num_batches} - "
                        f"loss {logging_loss} - samples/second: {logging_speed}")
...
def _evaluate_after_epoch(args, encoder, mlp, crf, eval_data, scheduler, optimizer, prev_lr):
    ...
    if lr != prev_lr: print(f"change lr from {prev_lr} to {lr}")
    return score, lr
...
def train(args, encoder, mlp, crf, train_data, dev_data, category2mentions, label2tokens):
    ...
    print(f"# sentences in training set: {len(train_data)}")
    print(f"# sentences in development set: {len(dev_data)}")
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        if dev_score == scheduler.best or "best_epoch" not in args.result:
            ...
            print(f"No improvement since last {epoch - args.result['best_epoch']} epochs, "
                        f"best score is {scheduler.best}")
            if epoch - args.result["best_epoch"] >= args.early_stop_patience:
                ...
                print(f"Early stop since no improvement since last {args.early_stop_patience} epochs")
                break